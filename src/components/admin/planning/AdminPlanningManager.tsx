...
import { TrainingMigration } from "./TrainingMigration";
import { MacrocycleListView } from "./MacrocycleListView";
import { MacrocycleDetailView } from "./MacrocycleDetailView";
import {
  MacrocycleFormState,
  MesocycleFormState,
  MicrocycleFormState,
...
export const AdminPlanningManager = forwardRef<AdminPlanningManagerRef, {}>(
  (props, ref) => {
    ...
    const [editingMicrocycleId, setEditingMicrocycleId] = useState<
      string | null
    >(null);
    ...
    const openCreateMicrocycle = () => {
      setFeedback("Abre un macrociclo para crear microciclos.");
    };
    ...
    const handleMicrocycleSubmit = async (
      ...
      // Esta función solo se usa cuando se edita desde la vista de lista
      // La creación se maneja en MacrocycleDetailView
      ...
      try {
        const parentMicrocycle = microcycles.find(
          (m) => m.id === editingMicrocycleId
        );
        if (!parentMicrocycle?.mesocycle_id) {
          setFeedback("No se pudo determinar el mesociclo padre.");
          return;
        }

        const payload = {
          mesocycle_id: parentMicrocycle.mesocycle_id,
    ...
    const handleSessionSubmit = async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const payload = {
          macrocycle_id: selectedMacrocycleId,
          mesocycle_id: session?.mesocycle_id ?? undefined,
          microcycle_id: session?.microcycle_id ?? undefined,
    ...
    const duplicateMesocycle = async (mesocycle: Mesocycle) => {
      ...
      if (!macrocycleId) {
        setFeedback("No se pudo determinar el macrociclo padre.");
        return;
      }
      try {
    ...
    const duplicateMicrocycle = async (microcycle: Microcycle) => {
      const mesocycleId = microcycle.mesocycle_id;
      if (!mesocycleId) {
        setFeedback("No se pudo determinar el mesociclo padre.");
        return;
      }
    ...
    const handleMacroDelete = async (macrocycle: Macrocycle) => {
      if (
        !window.confirm(
          "¿Eliminar este macrociclo y todo su contenido asociado?"
        )
      ) {
        return;
      }
    ...
    const editorSubtitle = editor
      ? (() => {
          switch (editor.type) {
            ...
            case "session":
              return selectedMacrocycle
                ? `Plan asociado a ${selectedMacrocycle.name}`
                : "";
            default:
              return "";
          }
    ...
    const handleMigrationComplete = useCallback(async () => {
      await refreshMacrocycles();
      setFeedback("Migración completada. Los datos han sido actualizados.");
    }, [refreshMacrocycles]);

    const handleViewMacrocycle = (macrocycle: Macrocycle) => {
      setSelectedMacrocycleId(macrocycle.id);
      setViewMode("detail");
    };
    ...
    const handleCreateMesocycle = async (input: MesocycleFormState) => {
      if (!selectedMacrocycleId) return;
      await createMesocycle({
        macrocycle_id: selectedMacrocycleId,
    ...
    const handleUpdateMesocycle = async (
      id: string,
      input: MesocycleFormState
    ) => {
    ...
    const handleDuplicateMesocycle = async (id: string) => {
      const mesocycle = mesocycles.find((m) => m.id === id);
      if (!mesocycle) return;
    ...
    const handleCreateMicrocycle = async (input: MicrocycleFormState) => {
      // Necesitamos el mesocycle_id, esto se manejará en MacrocycleDetailView
      // Por ahora, placeholder
    };

    const handleUpdateMicrocycle = async (
      id: string,
      input: MicrocycleFormState
    ) => {
      // Placeholder
    };

    const handleDeleteMicrocycle = async (id: string) => {
      if (
        !window.confirm("¿Eliminar este microciclo y las sesiones asociadas?")
      ) {
        return;
    ...
    const handleDuplicateMicrocycle = async (id: string) => {
      const microcycle = microcycles.find((m) => m.id === id);
      if (!microcycle) return;
      await duplicateMicrocycle(microcycle);
      await refreshMacrocycles();
    };

    const handleCreateSession = async (input: SessionFormState) => {
      if (!selectedMacrocycleId) return;
      // La creación de sesiones se maneja en MacrocycleDetailView
      // Esta función es solo un placeholder
    };

    const handleUpdateSession = async (id: string, input: SessionFormState) => {
      const session = sessions.find((s) => s.id === id);
      await updateSessionPlan(id, {
        macrocycle_id: selectedMacrocycleId!,
        mesocycle_id: session?.mesocycle_id || undefined,
        microcycle_id: session?.microcycle_id || undefined,
        trainer_id: input.trainer_id || undefined,
    ...
    const handleUpdateMacrocycle = async (
      id: string,
      input: MacrocycleFormState
    ) => {
        ...
        <section className="relative flex flex-col gap-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <header className="flex flex-col gap-2">
            ...
            <p className="text-sm text-slate-600">
              Gestiona todas tus planificaciones de temporada. Haz clic en un
              macrociclo para ver y editar todos sus detalles.
            </p>
            {feedback ? (
          ...
          <TrainingMigration onComplete={handleMigrationComplete} />

          {/* Vista de lista de macrociclos */}
          <MacrocycleListView
            ...
            onEdit={(macrocycle) => {
              setSelectedMacrocycleId(macrocycle.id);
              setViewMode("detail");
              startMacroEdit(macrocycle);
            }}
            onView={handleViewMacrocycle}
